...
Исправлены все строковые артефакты (буквальные символы \n),
титульный лист оформлен строго по университетскому ГОСТ-стандарту МТУСИ.
"""
...
import sys
src_path = r"C:\Users\Слава\.gemini\antigravity\scratch\university-labs\information-ecology\lab-01-natural-lighting\src"
...
def add_p(doc, text="", space_before=0, space_after=0, align=WD_ALIGN_PARAGRAPH.JUSTIFY, indent=1.25, bold=False, italic=False, size=14, line_spacing=1.5):
...
def add_section_heading(doc, text: str):
    """Заголовок структурного элемента по ГОСТ 7.32 (14 пт, полужирный, черный)."""
    p = doc.add_paragraph()
...
def add_formula_block(doc, formula_text: str, num_str: str = ""):
    """Формула строго по центру, номер справа в скобках."""
...
def generate_gost_report():
    ...
    # Поля ГОСТ 7.32 (левое 30 мм, правое 15 мм, верх/низ 20 мм)
    ...
    # ==============================================================================
    add_p(doc, "МИНИСТЕРСТВО ЦИФРОВОГО РАЗВИТИЯ, СВЯЗИ И МАССОВЫХ КОММУНИКАЦИЙ", 
          space_before=0, space_after=1, align=WD_ALIGN_PARAGRAPH.CENTER, indent=0, bold=True, size=11, line_spacing=1.1)
    add_p(doc, "РОССИЙСКОЙ ФЕДЕРАЦИИ", 
          space_before=0, space_after=2, align=WD_ALIGN_PARAGRAPH.CENTER, indent=0, bold=True, size=11, line_spacing=1.1)
    add_p(doc, "Ордена Трудового Красного Знамени федеральное государственное бюджетное образовательное учреждение высшего образования",
          space_before=0, space_after=2, align=WD_ALIGN_PARAGRAPH.CENTER, indent=0, size=10, line_spacing=1.1)
    add_p(doc, "«МОСКОВСКИЙ ТЕХНИЧЕСКИЙ УНИВЕРСИТЕТ СВЯЗИ И ИНФОРМАТИКИ» (МТУСИ)",
          space_before=0, space_after=18, align=WD_ALIGN_PARAGRAPH.CENTER, indent=0, bold=True, size=12, line_spacing=1.1)

    add_p(doc, "Центр заочного обучения по программам бакалавриата",
          space_before=0, space_after=2, align=WD_ALIGN_PARAGRAPH.CENTER, indent=0, size=12, line_spacing=1.15)
    add_p(doc, "Кафедра «Экология, безопасность жизнедеятельности и электропитание»",
          space_before=0, space_after=54, align=WD_ALIGN_PARAGRAPH.CENTER, indent=0, size=12, line_spacing=1.15)
    ...
    add_p(doc, "по лабораторной работе № 1", space_before=0, space_after=4, align=WD_ALIGN_PARAGRAPH.CENTER, indent=0, bold=True, size=14, line_spacing=1.15)
    add_p(doc, "по дисциплине: «Информационная экология»", space_before=0, space_after=12, align=WD_ALIGN_PARAGRAPH.CENTER, indent=0, size=14, line_spacing=1.15)
    ...
    add_p(doc, "(Вариант № 6)", space_before=0, space_after=48, align=WD_ALIGN_PARAGRAPH.CENTER, indent=0, bold=True, size=14, line_spacing=1.15)

    # Аккуратный блок исполнителя и проверяющего (2-колоночная таблица без границ)
    ...
    add_sign_line(c_right, "Зачетная книжка: ЗБСТ25066")
    add_sign_line(c_right, "Направление: 09.03.02 Информационные системы и технологии")
    add_sign_line(c_right, "Профиль: Инженерия DevSecOps (2 курс)", space_after=12)
    add_sign_line(c_right, "Проверил:")
    add_sign_line(c_right, "доцент кафедры ЭБЖиЭ")
    add_sign_line(c_right, "Курбатов В.А.", bold=True, space_after=0)

    add_p(doc, "Москва, 2026 г.", space_before=64, space_after=0, align=WD_ALIGN_PARAGRAPH.CENTER, indent=0, size=12, line_spacing=1.0)
    ...
    add_p(doc, "2) рассчитать геометрические параметры помещения, световых проемов и взаимного расположения с противостоящим зданием;")
    ...
    t1_rows = [
        ("Разряд зрительной работы", "Разряд III", "Высокая точность (размер объекта 0,3–0,5 мм)"),
    ...
    add_formula_block(doc, "ρ_ср = (0,50 · 200,0 + 0,50 · 300,0 + 0,10 · 200,0) / 700,0 = 270,0 / 700,0 = 0,3857 (38,6 %).")
    ...
    add_p(doc, "1. Как нормируется естественная освещенность?", bold=True)
    add_p(doc, 
          ...
          "e = (E_внут / E_нар) · 100 %. Нормирование в относительных единицах обусловлено тем, что "
          "абсолютная наружная освещенность непрерывно меняется в широких пределах в зависимости от времени суток, "
          "сезона года и облачности, тогда как отношение внутренней освещенности к наружной остается относительно постоянной величиной.")

    add_p(doc, "2. Какие виды естественного освещения вы знаете?", bold=True)
    add_p(doc, 
    ...
    add_p(doc, "3. Какие требования предъявляются к системам производственного освещения?", bold=True)
    add_p(doc, "К системам производственного освещения предъявляются следующие обязательные требования:")
    add_p(doc, "• освещенность на рабочих местах должна строго соответствовать установленным санитарным нормам для данного разряда зрительной работы;")
    add_p(doc, "• обеспечение постоянства освещенности во времени, исключение пульсаций светового потока и стробоскопического эффекта;")
    add_p(doc, "• равномерное распределение яркости в поле зрения и ограничение слепящего действия (устранение прямой и отраженной блесткости);")
    add_p(doc, "• обеспечение благоприятного спектрального состава света, максимально приближенного к естественному дневному;")
    add_p(doc, "• отсутствие глубоких и резких теней на рабочих поверхностях оборудования;")
    add_p(doc, "• электро-, пожаро- и взрывобезопасность осветительных установок, надежность и простота обслуживания.")

    add_p(doc, "4. По каким параметрам определяется разряд зрительной работы?", bold=True)
    add_p(doc, "Разряд зрительной работы определяется в соответствии с нормативными таблицами СП 52.13330 по следующим критериям:")
    add_p(doc, "1) наименьший эквивалентный размер объекта различения (в миллиметрах). Установлены разряды от I (наивысшая точность, менее 0,15 мм) до VI (грубая работа, более 5 мм);")
    add_p(doc, "2) контраст объекта различения с фоном K = |B_о - B_ф| / B_ф (большой при K > 0,5, средний при K = 0,2...0,5 и малый при K < 0,2);")
    add_p(doc, "3) характеристика фона (светлый при коэффициенте отражения фона ρ > 0,4, средний при ρ = 0,2...0,4 и темный при ρ < 0,2).")

    add_p(doc, "5. Что характеризует «спектральная видность», в чем заключается ее особенность? Энергетические и фотометрические величины.", bold=True)
    add_p(doc, 
    ...
    print(f"Отчет успешно сохранен без артефактов: {target_path}")
